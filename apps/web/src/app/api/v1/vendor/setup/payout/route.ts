import { updateVendorSetupPayout } from "@mlm/domain";
import { VendorSetupPayoutSchema } from "@mlm/shared";
...
export async function PATCH(request: NextRequest) {
  ...
  const parsed = VendorSetupPayoutSchema.safeParse(raw);
  if (!parsed.success) {
    return NextResponse.json({ error: "Validation failed", issues: parsed.error.flatten() }, { status: 400 });
  }

  const setup = await updateVendorSetupPayout(auth.vendorId, parsed.data);
