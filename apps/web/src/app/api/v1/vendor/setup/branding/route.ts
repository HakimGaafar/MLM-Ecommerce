import { updateVendorSetupBranding } from "@mlm/domain";
import { VendorSetupBrandingSchema } from "@mlm/shared";
...
export async function PATCH(request: NextRequest) {
  ...
  const parsed = VendorSetupBrandingSchema.safeParse(raw);
  if (!parsed.success) {
    return NextResponse.json({ error: "Validation failed", issues: parsed.error.flatten() }, { status: 400 });
  }

  const setup = await updateVendorSetupBranding(auth.vendorId, parsed.data);
