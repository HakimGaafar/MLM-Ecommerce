...
import AdminCatalogBannersForm from "./AdminCatalogBannersForm";

export default async function AdminCatalogBannersPage() {
  const session = await requirePageAuth("ADMIN");
  if (!userHasSuperAdminRole(session.roles)) redirect("/admin");

  const locale = await getCustomerPreferredLocale();
  const dict = locale === "ar" ? ar : en;
  const ui = dict.adminCatalogBanners;
